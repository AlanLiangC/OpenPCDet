...
class VoxelNeXtHeadPlusPLus(PointHeadTemplate):
    """
    ...
    def __init__(self, model_cfg, input_channels, num_class,
        ...
        self.channel_att = ChannelAttention(in_planes=self.model_cfg.INPUT_DIM)
        ##******************************************##
        ##############################################
    ...
    def get_loss(self, tb_dict=None):
        ...
        center_loss_reg, tb_dict_3 = self.get_vote_loss_loss() # center assign 0.0097
        ...
        if self.model_cfg.LOSS_CONFIG.get('LOSS_INS', None) is not None:
            ...
            sa_loss_cls, tb_dict_0 = self.get_sa_ins_layer_loss() # 0.1532
            tb_dict.update(tb_dict_0)
        else:
            sa_loss_cls = 0

        # cls loss
        center_loss_cls, tb_dict_4 = self.get_center_cls_layer_loss() # 0.2125
        ...
        if self.model_cfg.TARGET_CONFIG.BOX_CODER == 'PointResidualCoder':
            ...
            center_loss_box, tb_dict_5 = self.get_center_box_binori_layer_loss() # 0.1106
        tb_dict.update(tb_dict_5)    
        
        # corner loss
        if self.model_cfg.LOSS_CONFIG.get('CORNER_LOSS_REGULARIZATION', False):
            corner_loss, tb_dict_6 = self.get_corner_layer_loss() # 0.0211
        ...
        point_loss = center_loss_reg + center_loss_cls + center_loss_box + corner_loss + sa_loss_cls + iou3d_loss + heat_map_loss# + center_reg_loss# + two_center_label_loss
    ...
    def _get_predicted_boxes(self, sp_features, pred_dicts, voxel_indices):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            xs = (spatial_indices[:, 1:2] + 0.5) * self.feature_map_stride * self.voxel_size[0] + self.point_cloud_range[0]
            ys = (spatial_indices[:, 0:1] + 0.5) * self.feature_map_stride * self.voxel_size[1] + self.point_cloud_range[1]

            box_part_list = [batch_index.reshape(-1,1),xs, ys, -torch.ones_like(ys)]
            ...
            for bs_idx in range(self.batch_size):
                ...
                _, index = torch.topk(sub_heatmap, 256)
                # index = sub_heatmap > 0.1
    ...
    def _gather_feat(self, sp_feat, centers, feature_map_stride = 8):
        """Gather feature map.

        Given feature map and index, return indexed feature map.

        Args:
            feat (torch.tensor): Feature map with the shape of [B, C, H*W].
        ...
        Returns:
            torch.Tensor: Feature map after gathering with the shape
                of [B, C, M].
        """
        H, W = sp_feat.spatial_shape
        feature_dim = sp_feat.features.shape[-1]
        dense_feature = sp_feat.dense().contiguous()
        dense_feature = dense_feature.view(self.batch_size, feature_dim, H*W)

        x, y = centers[..., 0], centers[..., 1]
        coord_x = (x - self.point_cloud_range[0]) / self.voxel_size[0] / feature_map_stride
        coord_y = (y - self.point_cloud_range[1]) / self.voxel_size[1] / feature_map_stride

        coord_x = torch.clamp(coord_x, min=0, max=W - 0.5)  # bugfixed: 1e-6 does not work for center.int()
        coord_y = torch.clamp(coord_y, min=0, max=H - 0.5)  #

        center = torch.cat((coord_x[..., None], coord_y[..., None]), dim=-1)
        center_int = center.int()
        center_int_float = center_int.float()
        ind = center_int_float[..., 1] * W + center_int_float[..., 0]
        new_features = gather_points(dense_feature, ind.int()).contiguous()
        return new_features.permute(0,2,1).contiguous().view(-1, new_features.shape[1])
    ...
    def forward(self, batch_dict):
        ...
        self.ins_point = batch_dict['encoder_xyz'][4]
        ...
        # _, sp_center_cls_preds, _ = self._get_predicted_boxes(x_conv.features, pred_dicts, voxel_indices)
        # ret_dict['sp_center_cls_preds'] = sp_center_cls_preds
        pred_centers = batch_dict['centers'].view(self.batch_size, -1, 4).contiguous()
        if not self.training:                                                                                          
            sp_pred_centers, sp_center_cls_preds, sp_pred_centers_features = self._get_predicted_boxes(x_conv.features, pred_dicts, voxel_indices)
            pred_centers = torch.cat([pred_centers, sp_pred_centers], dim=1)
        ...
        sp_center_features = self._gather_feat(x_conv, ctr_xyz)
        center_features = torch.cat([center_features, sp_center_features], dim=-1)
        center_features = self.channel_att(center_features)
        #*************************************************#
        ###################################################
