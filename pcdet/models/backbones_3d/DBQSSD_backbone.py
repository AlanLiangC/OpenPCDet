...
class DBQSSD_Backbone(nn.Module):
    """ Backbone for DBQ-SSD"""
    ...
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        self.pre_aggregation_mlps = sa_config.get('PRE_AGGREGATION_MLPS', [False for _ in range(len(sa_config.NSAMPLE_LIST))])
        ...
        self.sa_dynamic = sa_config.get('DYNAMIC', [False for _ in range(len(sa_config.NSAMPLE_LIST))])
        self.sa_dynamic_cost = sa_config.get('DYNAMIC_COST', [None for _ in range(len(sa_config.NSAMPLE_LIST))])
        self.sa_dynamic_activate = sa_config.get('DYNAMIC_ACTIVATE', 'gumbel')
        self.sa_dynamic_group = sa_config.get('DYNAMIC_GROUP', [False for _ in range(len(sa_config.NSAMPLE_LIST))])
        ...
        for k in range(sa_config.NSAMPLE_LIST.__len__()):
            ...
            if self.layer_types[k] == 'SA_Layer':
                ...
                self.SA_modules.append(
                    pointnet2_modules.DynamicPointnetSAModuleMSG_WithSampling(
                        ...
                        dynamic=self.sa_dynamic[k],
                        dynamic_cost=self.sa_dynamic_cost[k],
                        dynamic_activate=self.sa_dynamic_activate,
                        dynamic_group=self.sa_dynamic_group[k],
                        pre_aggregation_mlp=self.pre_aggregation_mlps[k],
    ...
    def forward(self, batch_dict):
        ...
        for i in range(len(self.SA_modules)):
            ...
            if self.layer_types[i] == 'SA_Layer':
                ...
                ctr_features = encoder_features[self.layer_inputs[self.ctr_idx_list[i] - 1]] if self.ctr_idx_list[i] != -1 else None
                li_xyz, li_features, li_cls_pred = self.SA_modules[i](xyz_input, feature_input, li_cls_pred, ctr_xyz=ctr_xyz, ctr_features=ctr_features)
